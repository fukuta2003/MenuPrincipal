    ...
    class Pedido : Db
    {
        ...
        public bool Consulta(int pid)
        {
            ...
            using (SqlCommand cmd = new SqlCommand(StrQuery, conn))
            {
                ...
                if (dr.HasRows)
                {
                    ...
                    if(dr["faturado"].ToString()=="S") 
                    {
                        Faturado = dr["faturado"].ToString();
                    } else
                    {
                        Faturado = "N";
                    }
            ...
            StrQuery = "dbo.consultaprodutospedido";
        ...
        public bool Salvar(bool pIncluir)
        {
            ...
            if (ret)
            {
                ...
                {
                    StrQuery = "SELECT max([Id]) as ultimo FROM[dbo].[PEDIDO]";
                    using (SqlCommand cmd = new SqlCommand(StrQuery, conn))
                    {
                        cmd.CommandType = System.Data.CommandType.Text;
                        dr = cmd.ExecuteReader();
                ...
                if (!pIncluir)
                {
                    ...
                    { // somente se tiver produtos é que ele vai deletar
                        StrQuery = "DELETE FROM PEDIDO_COMPL WHERE PEDIDO_Id=" + Id.ToString();
                ...
                if (ret)
                {
                    ...
                    foreach (ListViewItem item in PedDadosPro.Items)
                    {
                        if (pIncluir)
                        {
                            Id = Ultimo_ID;
                        ...
                        StrQuery = "INSERT INTO PEDIDO_COMPL (PEDIDO_id," +
                        ...
                        {
                            MessageBox.Show("Erro ao SALVAR PEDIDO->PRODUTOS Exceção: " + ex.ToString());
                            ret = false;
                        }
        ...
        public bool Excluir(int pId)
        {
            ...
            StrQuery = "DELETE FROM PEDIDO_COMPL WHERE ID=" + pId.ToString();
            ...
            StrQuery = "DELETE FROM PEDIDO WHERE ID=" + pId.ToString() + "";
        ...
        public bool FaturaPedido( int pPedido)
        {
            ret = false;
            if (!Conecta())
            {
            ...
            string StrQuery = "update pedido set faturado='S' where id=" + pPedido.ToString();
            using (SqlCommand cmd = new SqlCommand(StrQuery, conn))
            {
                cmd.CommandType = System.Data.CommandType.Text;
                try
